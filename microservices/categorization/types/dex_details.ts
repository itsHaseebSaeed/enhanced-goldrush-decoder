...
import {
    ...
    NftSalesReport,
    LendingReport,
} from "../transaction_service_types";
import {
    DECODED_ACTION,
    DECODED_EVENT_CATEGORY,
} from "../../../services/decoder/decoder.constants";

export const mapDexEventToReport = (
    decodedEvent: EventType
): DexReport | null => {
    const commonFields = (
        inputToken: EventToken | undefined,
        outputToken: EventToken | undefined
    ) => ({
        action: decodedEvent.action,
        protocol: decodedEvent.protocol,
        aggregator_name:
            decodedEvent.details.find((d) => d.heading === "Aggregator")
                ?.value ?? "",
        aggregator_address:
            decodedEvent.details.find((d) => d.heading === "Aggregator Address")
                ?.value ?? "",
        // version: 0,
        // fork_version: 0,
        // fork: "",
        ...
        pair_address:
            decodedEvent.details.find((d) => d.heading === "Pair")?.value ?? "",
        pair_lp_fee_bps: 0,
        lp_token_address:
            decodedEvent.details.find((d) => d.heading === "Lp Token Address")
                ?.value ?? "",
        lp_token_ticker:
            decodedEvent.details.find((d) => d.heading === "Token ID")?.value ??
            "",
        ...
        token_0_address: inputToken?.address ?? "",
        ...
        token_1_address: outputToken?.address ?? "",
        ...
        sender:
            decodedEvent.details.find((d) => d.heading === "Sender")?.value ??
            "",
        recipient:
            decodedEvent.details.find((d) => d.heading === "To")?.value ?? "",
    });
